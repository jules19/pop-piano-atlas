...
for (const arr of R.ARRANGEMENTS) {
  ...
  for (const sec of arr.sections) {
    ...
    if (sec.fill) assert.ok(/^(walkup|lift|walkup\+lift|pianoman|stab)$/.test(sec.fill), `${arr.id}/${sec.name}: bad fill "${sec.fill}"`);
    if (sec.colour) assert.ok(/^(subtle|rich)$/.test(sec.colour), `${arr.id}/${sec.name}: bad colour "${sec.colour}"`);
    if (sec.voicing) assert.ok(/^(add9|add2)$/.test(sec.voicing), `${arr.id}/${sec.name}: bad voicing "${sec.voicing}"`);
  }
}
...
// ---- passing chords (colour): the connector engine follows the playbook recipes
{
  const C = T.parseChord('C'), F = T.parseChord('F'), G = T.parseChord('G');
  const Dm = T.parseChord('Dm'), Am = T.parseChord('Am');
  // subtle: C → C/E → F (bass walks 1-3-4)
  const up = T.connectorFor(C, F, 'subtle');
  assert.strictEqual(up.kind, 'walkup');
  assert.strictEqual(up.chord.bassPc, 4, 'walkup bass should be E');
  assert.strictEqual(up.chord.rootPc, 0, 'walkup keeps the current harmony');
  // subtle: F → C/E → C (bass steps down onto the target's third)
  const down = T.connectorFor(F, C, 'subtle');
  assert.strictEqual(down.kind, 'walkdown');
  assert.strictEqual(down.chord.rootPc, 0);
  assert.strictEqual(down.chord.bassPc, 4);
  // rich: G → G7/B → C (secondary dominant, leading tone in the bass)
  const dom = T.connectorFor(G, C, 'rich');
  assert.strictEqual(dom.kind, 'dominant');
  assert.strictEqual(dom.chord.rootPc, 7);
  assert.strictEqual(dom.chord.bassPc, 11, 'dominant connector walks in on the leading tone');
  // rich: C → C#°7 → Dm (chromatic passing diminished between whole-step roots)
  const dim = T.connectorFor(C, Dm, 'rich');
  assert.strictEqual(dim.kind, 'dim');
  assert.strictEqual(dim.chord.bassPc, 1, 'passing dim sits on the semitone between');
  // no connector when the harmony doesn't move
  assert.strictEqual(T.connectorFor(C, C, 'rich'), null);
  assert.strictEqual(T.connectorFor(C, F, 'off'), null);
  // rendered: quarter-pulse over C | F with subtle colour puts an E in the bass at beat 4
  const qp = A.PATTERNS.find((p) => p.id === 'quarter-pulse');
  const ctx = E.buildContext('C | F', qp);
  const events = E.renderBar(ctx, qp, 0, 'verse', { colour: 'subtle' });
  const conn = events.find((e) => e.connector && e.hand === 'lh');
  assert.ok(conn && conn.step === 12, 'connector bass lands on beat 4');
  assert.strictEqual(conn.midis[0] % 12, 4, 'connector bass should be E (C/E walking to F)');
  assert.ok(!events.some((e) => e.hand === 'lh' && !e.connector && e.step >= 12), 'original LH tail cleared');
  // plain render is untouched
  const plain = E.renderBar(ctx, qp, 0, 'verse');
  assert.ok(!plain.some((e) => e.connector), 'no colour → no connectors');
  // pedal-point refuses connectors: static bass is its whole point
  const pp = A.PATTERNS.find((p) => p.id === 'pedal-point');
  const ctx2 = E.buildContext('C | F', pp);
  assert.ok(!E.renderBar(ctx2, pp, 0, 'verse', { colour: 'rich' }).some((e) => e.connector));
  // rich mode re-voices or adds an RH shell so the passing harmony is heard
  const bed = A.PATTERNS.find((p) => p.id === 'bedrock');
  const ctx3 = E.buildContext('C | Dm', bed);
  const rich = E.renderBar(ctx3, bed, 0, 'verse', { colour: 'rich' });
  const rhConn = rich.find((e) => e.connector && e.hand === 'rh');
  assert.ok(rhConn, 'held pattern should gain a soft RH shell on the passing dim');
  const dimPcs = new Set([1, 4, 7, 10]); // C#°7
  assert.ok(rhConn.midis.every((m) => dimPcs.has(m % 12)), 'shell spells the passing chord');
}

// ---- grace slides: a soft chromatic crush just before the hit, only on major thirds
{
  const lb = A.PATTERNS.find((p) => p.id === 'late-bloom');
  const ctx = E.buildContext('C | Am', lb);
  const major = E.renderBar(ctx, lb, 0, 'verse');
  const grace = major.find((e) => e.grace);
  const main = major.find((e) => !e.grace && e.hand === 'rh');
  assert.ok(grace, 'late-bloom should crush into the third on C');
  assert.strictEqual(grace.midis[0] % 12, 3, 'crush note is the ♭3');
  assert.ok(grace.step < main.step && grace.step >= 0, 'crush lands just ahead of the chord');
  assert.ok(grace.vel < main.vel, 'crush is quieter than the chord');
  const minor = E.renderBar(ctx, lb, 1, 'verse');
  assert.ok(!minor.some((e) => e.grace), 'no crush into a minor third');
}

// ---- pianoman fill: dyads with the fifth on top, closing on the coming chord
{
  const qp = A.PATTERNS.find((p) => p.id === 'quarter-pulse');
  const ctx = E.buildContext('C | G | Am | F', qp);
  const events = E.renderBar(ctx, qp, 3, 'verse', { fill: { type: 'pianoman', targetPc: 0 } });
  const dyads = events.filter((e) => e.fill && e.hand === 'rh');
  assert.strictEqual(dyads.length, 3, 'pianoman adds three dyads');
  for (const d of dyads.slice(0, 2)) {
    assert.strictEqual(d.midis.length, 2);
    assert.strictEqual((d.midis[1] - ctx.segments[3].chord.rootPc + 1200) % 12, 7, 'fifth stays on top over F');
  }
  const last = dyads[2];
  assert.ok(last.intoNextBar, 'final dyad anticipates the next bar');
  assert.ok(last.midis.every((m) => [0, 4, 7].includes(m % 12)), 'final dyad belongs to the coming C');
}

// ---- stab fill: beat 1 lands, then silence
{
  const wc = A.PATTERNS.find((p) => p.id === 'woodchop');
  const ctx = E.buildContext('C | G | Am | F', wc);
  const events = E.renderBar(ctx, wc, 3, 'verse', { fill: { type: 'stab', targetPc: 0 } });
  assert.ok(events.length > 0);
  assert.ok(events.every((e) => e.step < 4), 'stop-time: nothing after beat 1');
  const plain = E.renderBar(ctx, wc, 3, 'verse');
  assert.ok(plain.length > events.length, 'stab silences most of the bar');
}

// ---- voicing override: an arrangement section can brighten plain triads
{
  const bed = A.PATTERNS.find((p) => p.id === 'bedrock');
  const ctx = E.buildContext('C | G', bed);
  const bright = E.renderBar(ctx, bed, 0, 'verse', { colorOverride: 'add9' });
  const rh = bright.find((e) => e.hand === 'rh');
  assert.ok(rh.midis.some((m) => m % 12 === 2), 'add9 override should add the 9th to C');
}
